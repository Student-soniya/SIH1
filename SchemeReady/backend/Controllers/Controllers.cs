...
public class LegacyAuthController : ControllerBase
{
    ...
    public async Task<ActionResult<SmsOtpResponse>> SendSmsOtp([FromBody] SendSmsOtpRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.PhoneNumber) || request.PhoneNumber.Length < 10)
        {
            return BadRequest(new SmsOtpResponse { Success = false, Message = "Valid 10-digit Indian mobile number is required." });
        }
        ...
        string otp = !string.IsNullOrWhiteSpace(request.Otp) 
            ? request.Otp 
            : new Random().Next(100000, 999999).ToString();

        string masked = "+91 ******" + cleanPhone[^4..];

        // Configurable SMS Gateway Dispatch (Fast2SMS / Twilio)
        try
        {
            var config = HttpContext.RequestServices.GetService<IConfiguration>();
            string? apiKey = config?["SmsGateway:ApiKey"];
            if (!string.IsNullOrWhiteSpace(apiKey))
            {
                using var client = new HttpClient { Timeout = TimeSpan.FromSeconds(3) };
                string url = $"https://www.fast2sms.com/dev/bulkV2?authorization={apiKey}&route=otp&variables_values={otp}&numbers={cleanPhone}";
        ...
        return Ok(new SmsOtpResponse
        {
            Success = true,
            Message = $"OTP successfully dispatched to mobile number {masked} via SMS.",
            MaskedPhone = masked
        });
    }
}
