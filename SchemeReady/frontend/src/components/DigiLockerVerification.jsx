...
export default function DigiLockerVerification({ lang = 'en', profile }) {
  ...
  const text = {
    ...
    description: isHindi ? 'DigiLocker से सहमति देकर जारी दस्तावेज़ प्राप्त करें। केवल DigiLocker से सफलतापूर्वक प्राप्त दस्तावेज़ को DigiLocker सत्यापित दिखाया जाएगा।' : 'Give consent through DigiLocker to retrieve your issued documents. A document is shown as DigiLocker verified only after a successful DigiLocker response.',
  ...
  useEffect(() => {
    const onMessage = async (event) => {
      ...
      if (!txn) { setError(event.data.error || 'DigiLocker did not return a transaction ID.'); setStatus('error'); return; }
      setTransactionId(txn); setStatus('retrieving');
      try {
        const response = await tokenFetchJson('decentro/digilocker/documents', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ initialDecentroTransactionId: txn }) });
        const files = Array.isArray(response) ? response : (Array.isArray(response?.data) ? response.data : []);
        setIssuedFiles(files); setStatus(files.length ? 'verified' : 'empty');
      } catch (err) { setError(err?.message || 'Unable to retrieve DigiLocker documents.'); setStatus('error'); }
    };
  ...
  const startVerification = async () => {
    setError(null); setIssuedFiles([]); setStatus('starting');
    try {
      ...
      if (!popup) { window.location.href = session.authorizationUrl; return; }
      popupRef.current = popup; setStatus('waiting');
    } catch (err) { setError(err?.message || 'Unable to start DigiLocker verification. Configure the Decentro credentials and redirect URI on the backend.'); setStatus('error'); }
  };

  return <section className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 space-y-4">
    <div className="flex items-start justify-between gap-4"><div className="flex items-start gap-3"><div className="w-10 h-10 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center"><ShieldCheck className="w-5 h-5 text-blue-700" /></div><div><h3 className="text-base font-black text-slate-900">{text.title}</h3><p className="text-xs text-slate-500 mt-1 max-w-2xl">{text.description}</p></div></div>{status === 'verified' && <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-800 bg-emerald-100 border border-emerald-300 px-3 py-1.5 rounded-full"><CheckCircle2 className="w-3.5 h-3.5" />{text.verified}</span>}</div>
    ...
    {status !== 'verified' && status !== 'empty' && <button onClick={startVerification} disabled={['starting','waiting','retrieving'].includes(status)} className="bg-blue-700 hover:bg-blue-800 disabled:opacity-60 text-white px-4 py-2.5 rounded-xl text-xs font-bold inline-flex items-center gap-2">{['starting','waiting','retrieving'].includes(status) ? <Loader2 className="w-4 h-4 animate-spin" /> : <ShieldCheck className="w-4 h-4" />}<span>{status === 'retrieving' ? text.retrieving : status === 'starting' || status === 'waiting' ? text.connecting : text.connect}</span>{status === 'waiting' && <ExternalLink className="w-3.5 h-3.5" />}</button>}
